  ...
  describe 'scopes' do
    ...
    describe '.for_user' do
      it 'returns accounts for given user' do
        expect(described_class.for_user(user)).to include(active_account, inactive_account)
      end
    end

    describe '.by_type' do
      it 'returns accounts for given type' do
        expect(described_class.by_type('checking')).to include(checking_account)
        expect(described_class.by_type('checking')).not_to include(savings_account)
      end
    end
  end
    ...
    describe '#update_balance_from_transaction' do
      ...
      context 'with income transaction' do
        ...
        it 'increases current_balance' do
          ...
          end.to change(account, :current_balance).from(1000).to(1500)
        end
      end
      ...
      context 'with expense transaction' do
        ...
        it 'decreases current_balance' do
          expect do
            account.update_balance_from_transaction(transaction)
          end.to change(account, :current_balance).from(1000).to(700)
        end
      end
      ...
      context 'with transfer transaction (source account)' do
        ...
        let(:transaction) do
          build(:transaction, :transfer, user: user, account: account, transfer_account: target_account, amount: 200)
        end

        it 'decreases current_balance of source account' do
          expect do
            account.update_balance_from_transaction(transaction)
          end.to change(account, :current_balance).from(1000).to(800)
        end
      end
      ...
      context 'with transfer transaction (target account)' do
        ...
        let(:transaction) do
          build(:transaction, :transfer, user: user, account: source_account, transfer_account: target_account,
                                         amount: 200)
        end
